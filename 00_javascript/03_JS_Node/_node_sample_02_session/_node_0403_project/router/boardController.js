  ...
  app.get("/boardList", function (req, res) {
    ...
    if (log == null) {
      ...
      if (curPageNum % onepagepaingNumber > 0) {
        curPageBeginPagingNumber =
          parseInt(curPageNum / onepagepaingNumber) * onepagepaingNumber + 1;
      } else {
        curPageBeginPagingNumber =
          (parseInt(curPageNum / onepagepaingNumber) - 1) * onepagepaingNumber +
          1;
      }

      var curPageEndPagingNumber =
        curPageBeginPagingNumber + onepagepaingNumber - 1; // 현재 페이지의 페이징 마침번호
